...
\[\frac{\partial RSS}{\partial \beta_0} = 2 \sum \left(y_i - \beta_0 - \beta_1\right)(-1) \]
\[\frac{\partial RSS}{\partial \beta_1} = 2 \sum \left(y_i - \beta_0 - \beta_1 \right)(-1_f) \]
...
\[\sum \left(y_i - \hb_0 - \hb_1 \cdot 1_f\right) = k \bar{y}_f + (n - k) \bar{y}_m - n \hb_0 - k \hb_1 = 0\]
\[\sum \left(\left(y_i - \hb_0 - \hb_1 \cdot 1_f\right)\cdot 1_f\right) = k \bar{y}_f - k \hb_0 - k \hb_1 =0\]
...
Также можно заметить, что $\widehat{Corr}^2 = R^2$. То есть, выборочная корреляция равна $-1/2$, так как коэффициенты отрицательные (между \(y\) и \(x\) отрицательная зависимость).
\end{sol}
\end{problem}
