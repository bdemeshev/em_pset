...
\begin{sol} % 1.5.
$\hat{\alpha} + \hat{\gamma} = 0$ и $\hb + \hat{\delta} = 1$



\begin{enumerate}
\item Это одно и то же, просто названия разные.
\item Смотрим предыдущий пункт.
\end{enumerate}
...
\[
\tilde{y}_i = \frac{y_i-\bar{y}}{s_y} \Rightarrow \E[\tilde{y}] = 0, \hspace{2mm} \mathbb{V}ar(\tilde{y}) = 1
\]
\[
y^*_i = \tilde{y}_i\cdot3 + 7 \Rightarrow \E[y^*] = 7, \hspace{2mm} \mathbb{V}ar(y^*) = 9
\]
\end{sol}
\end{problem}
...
\begin{sol}
  $R^2 = -3 \cdot \frac{-1}{13}=\frac{1}{4}$. Выборочная корреляция равна $-1/2$, так как коэффициенты отрицательные.
\end{sol}
\end{problem}
