...
\[\text{sCorr}(\hy, y) = \sqrt{R^2} \approx  0.949  \]
\end{sol}
\end{problem}
...
Здесь $R^2=
<<"020_a", echo=FALSE, results="asis">>=
cat(round(report$r.squared, 2))
@
$, а $t=
<<"020_b", echo=FALSE, results="asis">>=
cat(round(report$coefficients[2,3], 2))
@
$.
...
$Y_i -  \overline{Y} + \overline{Y} = \hb_1 + \hb_2 (X_i -  \overline{X} + \overline{X}) + \he_i$

$Y_i -  \overline{Y}  = \underbrace{\hb_1 - \overline{Y} + \hb_2 \overline{X}}_{=0} + \hb_2 (X_i -  \overline{X}) + \he_i$

$Y_i -  \overline{Y} = \hb_2 (X_i -  \overline{X}) + \he_i$
...
$y = \hb_2 x + \he$, где $y = \begin{bmatrix}
y_1 & \ldots & y_n
...
$\hat{Y} - \overline{Y}_i = (H - \pi)Y = (I - \pi)(H - \pi)Y = (I-\pi)(\hat{Y} - \overline{Y}_i)$.
...
$X_i = \hb_3 + \hb_4 Y_i = (\overline{X} - \hb_4\overline{Y}) + \hb_4 Y_i = \left( \overline{X} - \frac{1}{\hb_2} \overline{Y} \right) + \frac{1}{\hb_2} Y_i$
