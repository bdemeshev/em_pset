...
\begin{enumerate}
\item Найдите $\hat{\beta_1}$, $\hat{\beta_2}$, $\Corr(\hat{\beta_1}, \hat{\beta_2})$
...
\item Оценки несмещенны, дисперсия задаётся стандартными формулами.
...
$\mathbf{P}(\cdot)$ & $\frac{1}{2}$ & $\frac{1}{2}$\\
\bottomrule
\end{tabular}
\end{center}
...
$\mathbf{P}(\cdot)$ & 0.5  & $\frac{1}{4}$ & $\frac{1}{4}$\\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
\item $\E(\e_2) = 0$, $\Cov(\e_1, \e_2) = 0$, $\E(\e_3^5) = 0$, как и все нечетные начальные моменты симметричного относительно нулевого матожидания распределения. Остатки регрессии так же будут распределены нормально, как линейная комбинация нормально распределенных случайных величин. $\E(e_i) = \E[(\beta_1-\hb_2) + (\beta_2-\hb_2)x_i + \e_i] = 0$, так как МНК оценки $\hb_1$ и $\hb_2$ — несмещенные. То есть все остатки распределены нормально с 0 матожиданием, поэтому все нечетные начальные моменты будут равны нулю. Отсюда: $\E(e_5^3) = 0$. $\Var(y_3) = \Var(\beta_1 + \beta_2x_3 + \e_3) = \Var(\e_3) = 9$.
...
Здесь $R^2=\Sexpr{round(report$r.squared, 2)}$, а $t=\Sexpr{round(report$coefficients[2,3], 2)}$.
...
\[\hb_1 = - \frac{\hat{\gamma}_1}{\hat{\gamma}_2}, \:\:\:\:\: \hat{\beta_2}= \frac{1}{\hat{\gamma}_2} \]
\[ |\text{sCorr}(x, y)| = 1 \]
...
\[ \frac{\hat{\gamma}_1}{\hat{\gamma}_2} = \frac{\bar{x}}{{\hat{\gamma}_2}} - \bar{y} =  \hat{\beta_2} \bar{x} - \bar{y} = \bar{y} - \hb_1 - \bar{y} = -\hat{\beta_1}  \]
...
$y = \hb_2 x + \hat{\e}$, где $y = \begin{bmatrix}
y_1 & \ldots & y_n
...
$x' y = \hb_2 x' x + \underbrace{x' \hat{\e}}_{=0}$
...
Далее, $\hat{Y} - \overline{Y}_i = (I - \pi)(\hat{Y} - \overline{Y}_i) = (I - \pi)(\hb_1 + \hb_2 X - \overline{Y}_i) = \hb_2 x$

Значит, $ESS = \hb_2^2 x' x$.

Получаем:
\[
\label{task20:corr}R^2 = \frac{ESS}{TSS} = \frac{\hb_2^2 x' x^{(2)}}{y' y} = \frac{x' y^{(2)}}{(x' x)(y' y)} = \Corr^2(X, Y)
\]
...
Теперь математическое ожидание от ошибок $\E(\tilde{\e}_i) = 0$, то есть предпосылки теоремы Гаусса-Маркова выполнены. Поэтому МНК оценки будут несмещенными, а дисперсию ошибок можно оценить как $\hat{\sigma}^2 = \frac{RSS}{n-2}$. Вспомним, что для пуассоновского распеределения $\E(X) = \Var(X) = \lambda$. Поэтому
\[\frac{RSS}{n-2} = \widehat{\Var}(\tilde{\e}_i) = \widehat{\Var}(\e_i - \lambda) = \widehat{\Var}(\e_i) = \hat{\lambda} \]
...
\begin{figure}
\begin{tikzpicture}[scale = 0.025]
\input{R_plots/two_points_zero_slope.tikz}
\end{tikzpicture}
\end{figure}
...
\begin{figure}
\begin{tikzpicture}[scale = 0.025]
\input{R_plots/two_points_positive_slope.tikz}
\end{tikzpicture}
\end{figure}
...
\begin{problem}
  Три точки, прямая $\hy_i = \hb_1 + \hb_2 x_i$. Даны два расстояния, надо найти третье.
\begin{sol}
\end{sol}
\end{problem}
...
\begin{tikzpicture}[scale = 0.025]
\input{R_plots/regression_on_plot.tikz}
\end{tikzpicture}
\end{minipage}
...
\begin{minipage}{\textwidth}
\begin{tikzpicture}[scale = 0.025]
\input{R_plots/4_observations.tikz}
\end{tikzpicture}
\end{minipage}
...
\item Теоретическую дисперсию $\Var(X)$ и выборочную дисперсию $\hVar(x)$.
\item Теоретическую ковариацию $\Cov(X, Y)$ и выборочную ковариацию $\hCov(x, y)$.
\item Теоретическую корреляцию $\Corr(X, Y)$ и выборочную корреляцию $\hCorr(x, y)$.
...
\item Как связаны между собой $\hCorr(X, Y)$, $\hb_2$ и $\hat\gamma_2$?
...
\item Теоретическую частную корреляцию $\pCorr(X, Y; Z)$ и выборочную частную корреляцию $\hpCorr(x, y; z)$.
\end{enumerate}
