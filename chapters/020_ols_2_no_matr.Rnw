...
<<>>=
qcrit1 <- qt(0.05/2, df = 5 - 2)
qcrit1
qcrit2 <- qt(1 - 0.05/2, df = 5 - 2)
qcrit2
@
...
\item
Заметим, что $\Var(\varepsilon_i) = \E(\varepsilon_i^2) = 1$, $\E(\varepsilon_1\varepsilon_2) = 0$. Поэтому:
\[
...
\item Как связаны между собой $\hCorr(X, Y)$, $\hat\beta_2$ и $\hat\gamma_2$?
