...
\begin{solution}
Оценка $\hb_2$ несмещённая, $\Var(\hb_2)=4\sigma^2/n$ при чётных $n$, и $\Var(\hb_2)=4n\sigma^2/(n^2-1)$ при нечётных $n$. В любом случае $\Var(\hb_2) \to 0$, следовательно оценка $\hb_2$ является состоятельной.
\end{solution}
...
\begin{solution}
$\hat{a} \approx 267$, $\hat{b} \approx 167$.
...
\begin{solution}
Суммы $TSS$ и $RSS$ только растут, $ESS$ и $R^2$ могут изменяться в обе стороны.
\end{solution}


\begin{problem}
Эконометресса Аглая подглядела, что у эконометрессы Жозефины получился $R^2$ равный $0.99$ по 300 наблюдениям. От чёрной зависти Аглая не может ни есть, ни спать. Поэтому она решила добавить в набор данных Жозефины ещё наблюдений так, чтобы $R^2$ упал до нуля. Можно ли этого добиться и если да, то как?
\end{problem}

\begin{solution}
добавить 300 противоположных наблюдений
\end{solution}
...
\begin{solution}
При всей бессмысленности, гипотеза $\beta_1=0$ --- это гипотеза о том, что средний вес женщин равен нулю. Гипотеза $\beta_2=0$ --- гипотеза о нулевой разнице ожидаемого веса мужчин и женщин.
\end{solution}
