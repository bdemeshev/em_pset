...
\]
где $q$ — количество ЛНЗ ограничений (в нашем случае $q=1$), $n$ — количество наблюдений (одинаковое для обеих моделей) и $k$ —  количество оцениваемых коэффициентов в неограниченной модели ($k=2$). Итак, $\beta_1 = 1-\beta_2$:
...
\begin{sol}
  ...
  \item Возьмём произвольный набор точек. Добавим точку «от фонаря». Помеяется наклон и новая прямая пересечёт старую. Перенесём вдоль оси $x$ точки так, чтобы новая точка пересечения легла на вертикальную ось.
  \end{enumerate}
\end{sol}
\end{problem}

\begin{problem}
  график. Две точки на прямой с нулевым наклоном. Найти все $\hy_i$, $RSS$ и прочее
\begin{sol}
\end{sol}
\end{problem}

\begin{problem}
  график. Две точки на прямой с положительным наклоном. Найти все $\hy_i$, $RSS$ и прочее
\begin{sol}
\end{sol}
\end{problem}
