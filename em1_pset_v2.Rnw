...
\ifdef{\JustPlot}{}{ % этот блок не включается в tikz-файлы по которым строятся графики
...
# library("tikzsetup") # devtools::install.github("bdemeshev/tikzsetup")
# library("tikzDevice")
# tikzsetup()
...
library("econru") # devtools::install.github("bdemeshev/econru")
